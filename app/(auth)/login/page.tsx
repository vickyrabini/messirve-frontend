...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    import("gsap").then(({ gsap }) => {
      const xTo1 = gsap.quickTo(glow1Ref.current, "x", {
        duration: 0.8,
        ease: "power1.out",
      });
      const yTo1 = gsap.quickTo(glow1Ref.current, "y", {
        duration: 0.8,
        ease: "power1.out",
      });
      const xTo2 = gsap.quickTo(glow2Ref.current, "x", {
        duration: 1.2,
        ease: "power1.out",
      });
      const yTo2 = gsap.quickTo(glow2Ref.current, "y", {
        duration: 1.2,
        ease: "power1.out",
      });
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-cream">
      <div className="min-h-full lg:grid lg:grid-cols-[1.05fr_1fr]">
        ...
        <aside className="relative hidden lg:flex flex-col overflow-hidden bg-celeste p-12 xl:p-16 text-white">
          ...
          <div
            ref={glow1Ref}
            className="float-slow pointer-events-none absolute -top-24 -left-20 h-[28rem] w-[28rem] rounded-full bg-white/10 blur-3xl"
          />
          <div
            ref={glow2Ref}
            className="float-slow pointer-events-none absolute bottom-0 right-0 h-96 w-96 rounded-full bg-dorado/25 blur-3xl"
          />
          <AuthSunDecor />
          ...
          <div className="reveal relative z-10">
            <Link
              href="/"
              className="inline-flex items-center rounded-2xl bg-white px-4 py-3 shadow-soft"
            >
              <Image
          ...
          <div className="relative z-10 flex flex-1 flex-col justify-center max-w-md">
            ...
            <p
              className="reveal mt-6 text-lg leading-relaxed text-white/90"
              style={{ transitionDelay: "60ms" }}
            >
              La comunidad argentina y uruguaya en Barcelona, a un toque.
              Reseñas de los que entienden de dónde venís.
            </p>
          </div>
          {/* Estrellas */}
          <div
            className="reveal relative z-10 flex items-center gap-2"
          ...
          <div className="flex flex-1 items-center justify-center px-6 py-10 sm:px-10">
            <div className="w-full max-w-[440px]">
              <header className="mb-8">
                ...
                <p
                  className="reveal mt-2 text-[15px] text-muted"
                  style={{ transitionDelay: "60ms" }}
                >
              ...
              <form action={formAction} noValidate className="space-y-5">
                ...
                <div className="reveal" style={{ transitionDelay: "180ms" }}>
                  <div className="mb-1.5 flex items-center justify-between">
                    ...
                    <Link
                      ...
                      className="text-[13px] font-semibold text-celeste-deep hover:underline"
                    >
                      ¿La olvidaste?
                    </Link>
                  </div>
                ...
                {state?.error && (
                  <div className="flex items-start gap-3 rounded-2xl border border-[#E8B4AC] bg-[#FBEAE7] px-5 py-4">
                    ...
                    <div>
                      <p className="text-[16px] font-semibold text-[#A63B24]">
                        No pudimos iniciar sesión
                      </p>
                      <p className="mt-0.5 text-[15px] text-[#B4665A]">
                        {state.error}
                      </p>
                    </div>
                  </div>
              ...
              <p
                className="reveal mt-6 text-center text-[14px] text-muted"
