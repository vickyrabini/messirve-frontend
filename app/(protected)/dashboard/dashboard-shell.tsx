...
import type { Category, ServiceWithStats, ClientRequestStatus } from '@/types/database'
    ...
    <div className="relative flex min-h-screen overflow-hidden bg-cream">
          ...
          {TABS.map((t) => (
            <button
              ...
              className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-[15px] font-semibold transition-colors ${
                tab === t.id ? 'bg-white/10 text-white' : 'text-white/55 hover:text-white/85'
              }`}
            >
              {t.icon}
              {t.label}
        ...
        <div className="mt-auto border-t border-white/10 pt-4">
          <div className="flex items-center gap-3 px-1.5 py-2">
            ...
            <div className="min-w-0">
              <p className="truncate text-[15px] font-semibold text-white">{fullName || 'Sin nombre'}</p>
      ...
      <main className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-10 flex h-[78px] shrink-0 items-center justify-between border-b border-gris/40 bg-cream/90 px-8 backdrop-blur">
          <h1 className="font-brand text-2xl uppercase text-ink">{TAB_TITLES[tab]}</h1>
          <div className="grid h-11 w-11 place-items-center rounded-full bg-celeste font-brand text-lg text-white">{initial}</div>
        </header>

        <div className="flex-1 px-8 py-9" style={{ maxWidth: '1240px' }}>
          {tab === 'explorar' && <ExplorarTab fullName={fullName} categories={categories} services={exploreServices} />}
          {tab === 'favoritos' && <FavoritosTab services={favoriteServices} />}
          {tab === 'resenas' && <ResenasTab reviews={reviews} />}
          {tab === 'suscripcion' && <SuscripcionTab role={role} ownService={ownService} clientRequest={clientRequest} />}
          {tab === 'perfil' && <PerfilTab fullName={fullName} email={email} memberSince={memberSince} />}
        </div>
      </main>
    </div>
  )
