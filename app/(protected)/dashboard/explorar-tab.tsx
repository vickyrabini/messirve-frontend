...
export function ExplorarTab({ fullName, categories, services }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return services.filter((s) => {
      const matchesCategory = !categorySlug || s.categories?.slug === categorySlug
  ...
  return (
    <div>
      ...
      <div className="mb-8 flex flex-wrap gap-3">
        <button
          ...
          className={`rounded-full px-4 py-2 text-[14px] font-semibold transition-colors ${
            categorySlug === null ? 'bg-celeste-deep text-white' : 'bg-white text-ink border border-gris/60 hover:border-celeste'
          }`}
        >
          Todos
        </button>
        {categories.map((c) => (
          <button
            key={c.id}
            type="button"
            onClick={() => setCategorySlug(c.slug === categorySlug ? null : c.slug)}
            className={`rounded-full px-4 py-2 text-[14px] font-semibold transition-colors ${
              categorySlug === c.slug ? 'bg-celeste-deep text-white' : 'bg-white text-ink border border-gris/60 hover:border-celeste'
            }`}
          >
            {c.emoji} {c.name}
          </button>
      ...
      <div className="mb-5 flex items-baseline justify-between">
        <h3 className="font-brand text-xl text-ink uppercase">Recomendados para vos</h3>
        <span className="text-[14px] text-muted">{filtered.length} servicios</span>
      </div>

      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-gris/40 bg-white p-10 text-center">
          <p className="text-4xl">🔍</p>
          <p className="mt-3 text-[15px] font-semibold text-ink">No encontramos nada con esa búsqueda</p>
          <p className="mt-1 text-sm text-muted">Probá con otra palabra o categoría</p>
        </div>
      ) : (
