    ...
    <div className="overflow-hidden rounded-[20px] bg-white shadow-card">
      <div className="relative h-[250px]">
        ...
        <div className="absolute inset-x-0 bottom-0 flex h-[110px] flex-col justify-end bg-gradient-to-t from-black/70 to-transparent px-3.5 pb-3">
          <p className="truncate text-[19px] font-bold leading-[25px] text-white">
            {service.name}
          </p>
        ...
        {category?.name && (
          <div className="absolute left-3 top-3 flex items-center gap-1 rounded-full bg-white/90 px-2.5 py-1">
            ...
            <span className="text-[10px] font-bold tracking-wide text-ink">
              {category.name}
            </span>
          </div>
        )}
        ...
        <form action={toggleLike} className="absolute right-2.5 top-2.5">
          ...
          <input
            type="hidden"
            name="liked"
            value={String(service.user_liked)}
          />
          <button
            type="submit"
            aria-label={
              service.user_liked ? "Quitar de favoritos" : "Agregar a favoritos"
            }
      ...
      <div className="bg-white px-3.5 pb-3.5 pt-3">
        <div className="flex items-center gap-1.5">
          {service.address && (
            <p className="flex-1 truncate text-[13px] text-muted">
              <span className="text-celeste">📍</span> {service.address},{" "}
              {service.city}
            </p>
          )}
          {service.price_info && (
        ...
        {service.description && (
          <p className="mt-2 line-clamp-2 text-[13px] leading-[19px] text-muted">
            {service.description}
          </p>
        )}
        ...
        <div className="mt-3 flex items-center justify-between border-t border-gris/30 pt-2.5">
          ...
          <Link
            ...
            className="text-[13px] font-bold text-celeste hover:underline"
          >
            Ver más →
          </Link>
        </div>
